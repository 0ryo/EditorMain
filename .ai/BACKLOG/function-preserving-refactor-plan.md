...
| 5. Scenario graph UI | Awaiting validation-text verification | `refactor: separate scenario graph layout`<br>`fix: make validation panel minimize reliable`<br>`refactor: separate scenario minimap layout`<br>`refactor: separate scenario validation text` | Node layout, validation panel and minimap passed; validation text pending |
