...
| 3. 3D editing | Selection, outliner, and transform passed; awaiting camera verification | `refactor: separate placed object selection processing`<br>`refactor: separate outliner data processing`<br>`refactor: separate transform gizmo geometry`<br>`refactor: separate camera navigation calculations`<br>`fix: restrict camera navigation to viewport`<br>`fix: ignore camera input outside game view` | Camera pending |
