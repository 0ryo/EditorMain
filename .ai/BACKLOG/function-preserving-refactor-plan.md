...
| 5. Scenario graph UI | Awaiting minimap verification | `refactor: separate scenario graph layout`<br>`fix: make validation panel minimize reliable`<br>`refactor: separate scenario minimap layout` | Node layout and validation panel passed; minimap pending |
