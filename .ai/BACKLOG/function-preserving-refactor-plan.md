...
| 4. Scenario domain/validation | Validation, connection rules, traversal, and export passed; awaiting command verification | `refactor: separate scenario graph validation`<br>`refactor: separate scenario connection rules`<br>`refactor: separate scenario graph traversal`<br>`refactor: separate scenario export building`<br>`refactor: separate scenario graph command processing` | Commands pending |
