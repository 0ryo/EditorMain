...
| 4. Scenario domain/validation | Validation, connection rules, and traversal passed; awaiting export verification | `refactor: separate scenario graph validation`<br>`refactor: separate scenario connection rules`<br>`refactor: separate scenario graph traversal`<br>`refactor: separate scenario export building` | Export pending |
